...
#ifdef CONFIG_MHL_PS5101_VE
static void ps5101_cfg_gpio(void)
{
	int ret;
	printk(KERN_INFO "%s()\n", __func__);
	s3c_gpio_cfgpin(GPIO_MHL_SDA_1_8V, S3C_GPIO_SFN(0x0));
	...
	printk(KERN_INFO "[MHL] %s MHL_WAKEUP_IRQ, IRQ_TYPE_EDGE_RISING \n", __func__);
	...
	printk(KERN_INFO "[MHL] %s MHL_INT_IRQ, IRQ_TYPE_EDGE_RISING \n", __func__);
...
static void ps5101_power_onoff(bool on)
{
#ifdef	CONFIG_MACH_TAB3
	struct regulator *regulator_1_2v;
	struct regulator *regulator_1_8v;
...
#ifdef __MHL_NEW_CBUS_MSC_CMD__
#if defined(CONFIG_MFD_MAX77693)
static int ps5101_usb_op(bool on, int value)
{
	pr_info("func:%s bool on(%d) int value(%d)\n", __func__, on, value);
	printk(KERN_INFO "[MHL] + Midas-mhl.c : %s \n",__func__);
	if (on) {
...
#endif
static void ps5101_vbus_present(bool on, int value)
{
	struct power_supply *psy = power_supply_get_by_name(PSY_CHG_NAME);
	union power_supply_propval power_value;
	u8 intval;
...
#endif
#ifdef CONFIG_SAMSUNG_MHL_UNPOWERED
static int ps5101_get_vbus_status(void)
{
	struct power_supply *psy = power_supply_get_by_name(PSY_BAT_NAME);
	union power_supply_propval value;
	u8 intval;
...
static void ps5101_otg_control(bool onoff)
{
	printk(KERN_INFO "[MHL] + Midas-mhl.c : %s \n",__func__);
	otg_control(onoff);
	gpio_request(GPIO_OTG_EN, "USB_OTG_EN");
...
static void ps5101_reset(void)
{
 ...
 int get_mhl_ps5101_int_irq(void)
{
...
struct ps5101_platform_data ps5101_pdata = { 
	.get_irq = get_mhl_ps5101_int_irq,
	.hw_onoff = ps5101_power_onoff,
	.hw_reset = ps5101_reset,
	.gpio_cfg = ps5101_cfg_gpio,
	.vbus_present = muic_mhl_cb,
	.ps5101_muic_cb = muic_mhl_cb,
	.pageno = 0,
	.offset = 0,
	.ddc_i2c_num = 0,
};

static struct i2c_board_info __initdata i2c_devs_sii9234_ps5101[] = {
	/* ps5101 */
	{
	        I2C_BOARD_INFO("ps5101_p0", PS5101I2CBASEADDR >> 1 ),   // 0x90 or 0x10
	        .platform_data = &ps5101_pdata,
	},
	{
	        I2C_BOARD_INFO("ps5101_p1", ( PS5101I2CBASEADDR + 0x02 ) >> 1 ),        // 0x92 or 0x12
	        .platform_data = &ps5101_pdata,
	},

	{
	        I2C_BOARD_INFO("ps5101_p2", ( PS5101I2CBASEADDR + 0x04 ) >> 1 ),        // 0x94 or 0x14
	        .platform_data = &ps5101_pdata,
	},

	{
	        I2C_BOARD_INFO("ps5101_p3", ( PS5101I2CBASEADDR + 0x06 ) >> 1 ),        // 0x96 or 0x16
	        .platform_data = &ps5101_pdata,
	},
       /* sii9234 */
	{
...
#ifdef CONFIG_MHL_PS5101_VE  // for dualization
	ret = i2c_add_devices(I2C_BUS_ID_MHL, i2c_devs_sii9234_ps5101,
				ARRAY_SIZE(i2c_devs_sii9234_ps5101));
...
#endif
#ifdef CONFIG_MHL_PS5101_VE  // for dualization
	ps5101_pdata.ddc_i2c_num = sii9234_pdata.ddc_i2c_num;
#endif
			pr_info( "[MHL] sii9234_pdata.ddc_i2c_num = %d\n", sii9234_pdata.ddc_i2c_num);
